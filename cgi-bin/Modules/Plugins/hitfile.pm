...
sub login {
	...
	$self->get('http://hitfile.net/');
	my $req = POST "http://hitfile.net//user/login",
	        Referer => "http://hitfile.net/",
	       	Content => ['user[login]'=>$a->{login}, 'user[pass]'=>$a->{password}, returnto=>'/'];	
	$self->request($req);
	if(($self->{response}->is_success) || ($self->{response}->code == 302)) {
		$self->get('http://hitfile.net/?cookiecheck=1');
...
sub download {
	...
	$self->get($url);
	($dlink) = $self->{content} =~ m~(http://hitfile.net//?download[^\'\"]*)~s;
	if($dlink) {
		$req = GET $dlink;
